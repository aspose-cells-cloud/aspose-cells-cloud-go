...
func (request *UpdateWordCaseInRemoteSpreadsheetRequest) Validate() error {
	if request.name == "" {
	...
	if request.wordCaseType == "" {
		return fmt.Errorf("required request parameter %q is missing", "wordCaseType")
	}
