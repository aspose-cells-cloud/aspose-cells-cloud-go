...
func (request *PostFooterRequest) Validate() error {
	if request.name == "" {
		return fmt.Errorf("required request parameter %q is missing", "name")
	}

	if request.script == "" {
		return fmt.Errorf("required request parameter %q is missing", "script")
	}
...
func (request *PostFooterRequest) AddQueryParameter(key, value string) {
...
func (request *PostFooterRequest) AddQueryParameters(params map[string]string) {
	if request.extraQueryParameters == nil {
		request.extraQueryParameters = make(map[string]string)
	}
