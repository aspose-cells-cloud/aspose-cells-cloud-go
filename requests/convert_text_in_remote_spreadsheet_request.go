...
func (request *ConvertTextInRemoteSpreadsheetRequest) Validate() error {
	if request.convertTextType == "" {
		return fmt.Errorf("required request parameter %q is missing", "convertTextType")
	}
