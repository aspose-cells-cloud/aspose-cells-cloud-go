...
func (request *PostWorksheetCommentRequest) Validate() error {
	if request.cellName == "" {
		return fmt.Errorf("required request parameter %q is missing", "cellName")
	}

	if request.comment == nil {
		return fmt.Errorf("required request parameter %q is missing", "comment")
	}
...
func (request *PostWorksheetCommentRequest) AddQueryParameter(key, value string) {
...
func (request *PostWorksheetCommentRequest) AddQueryParameters(params map[string]string) {
	if request.extraQueryParameters == nil {
		request.extraQueryParameters = make(map[string]string)
	}
