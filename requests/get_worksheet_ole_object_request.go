...
func (request *GetWorksheetOleObjectRequest) Validate() error {
	if request.name == "" {
...
func (request *GetWorksheetOleObjectRequest) GetPath() string {
	...
	localVarPath = strings.Replace(localVarPath, "{"+"objectNumber"+"}", url.PathEscape(fmt.Sprintf("%v", request.objectNumber)), -1)
	return localVarPath
}
