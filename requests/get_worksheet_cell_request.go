...
func (request *GetWorksheetCellRequest) Validate() error {
	if request.cellOrMethodName == "" {
		return fmt.Errorf("required request parameter %q is missing", "cellOrMethodName")
	}
...
func (request *GetWorksheetCellRequest) AddQueryParameter(key, value string) {
...
func (request *GetWorksheetCellRequest) AddQueryParameters(params map[string]string) {
	if request.extraQueryParameters == nil {
		request.extraQueryParameters = make(map[string]string)
	}
...
func (request *GetWorksheetCellRequest) GetPath() string {
	...
	localVarPath = strings.Replace(localVarPath, "{"+"cellOrMethodName"+"}", url.PathEscape(fmt.Sprintf("%v", request.cellOrMethodName)), -1)
	return localVarPath
}
