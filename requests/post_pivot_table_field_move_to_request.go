...
func (request *PostPivotTableFieldMoveToRequest) Validate() error {
	if request.from == "" {
		return fmt.Errorf("required request parameter %q is missing", "from")
	}
	...
	if request.to == "" {
		return fmt.Errorf("required request parameter %q is missing", "to")
	}
