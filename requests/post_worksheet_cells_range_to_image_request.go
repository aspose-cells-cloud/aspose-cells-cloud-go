...
func (request *PostWorksheetCellsRangeToImageRequest) Validate() error {
	if request.name == "" {
		return fmt.Errorf("required request parameter %q is missing", "name")
	}

	if request.rangeConvertRequest == nil {
		return fmt.Errorf("required request parameter %q is missing", "rangeConvertRequest")
	}
