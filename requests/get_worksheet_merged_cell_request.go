...
func (request *GetWorksheetMergedCellRequest) Validate() error {
	if request.name == "" {
...
func (request *GetWorksheetMergedCellRequest) AddQueryParameter(key, value string) {
...
func (request *GetWorksheetMergedCellRequest) AddQueryParameters(params map[string]string) {
	if request.extraQueryParameters == nil {
		request.extraQueryParameters = make(map[string]string)
	}
...
func (request *GetWorksheetMergedCellRequest) GetPath() string {
	...
	localVarPath = strings.Replace(localVarPath, "{"+"mergedCellIndex"+"}", url.PathEscape(fmt.Sprintf("%v", request.mergedCellIndex)), -1)
	return localVarPath
}
