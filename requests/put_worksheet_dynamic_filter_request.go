...
func (request *PutWorksheetDynamicFilterRequest) Validate() error {
	if request.dynamicFilterType == "" {
		return fmt.Errorf("required request parameter %q is missing", "dynamicFilterType")
	}
