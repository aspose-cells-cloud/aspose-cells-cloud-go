...
func (request *GetWorksheetPivotTableFilterRequest) Validate() error {
	if request.name == "" {
...
func (request *GetWorksheetPivotTableFilterRequest) GetPath() string {
	...
	localVarPath = strings.Replace(localVarPath, "{"+"filterIndex"+"}", url.PathEscape(fmt.Sprintf("%v", request.filterIndex)), -1)
	return localVarPath
}
